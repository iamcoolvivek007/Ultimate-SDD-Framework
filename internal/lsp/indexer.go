...
var (
	// Patterns are defined at package level to avoid recompilation in loops.
	// This improves performance by ~3.5x (7.6µs/op vs 26.6µs/op).
	...
	// Rust patterns
	rustFuncPattern   = regexp.MustCompile(`(?:pub\s+)?fn\s+(\w+)`)
...
func parseRustSymbols(content, file string) []Symbol {
	...
	for lineNum, line := range lines {
		...
		if matches := rustFuncPattern.FindStringSubmatch(line); matches != nil {
			sym := Symbol{
				Name: matches[1],
				Kind: "function",
				File: file,
